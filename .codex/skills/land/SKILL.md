...
# Preferred: use the Async Watch Helper below. The manual flow is a fallback
# when Python cannot run or the helper script is unavailable.
# Query optional Codex Review feedback once before watching CI. Codex reviews
# arrive as issue comments that start with "## Codex Review — <persona>". If
# comments are present, treat them like reviewer feedback: reply with a
# `[codex]` issue comment acknowledging the findings and whether you're
# addressing or deferring them. If no such comments are present, continue to CI
# and mergeability checks; repositories without a Codex Review workflow must not
# wait indefinitely here.
codex_review_comments=$(
  gh api repos/{owner}/{repo}/issues/"$pr_number"/comments \
    --jq '.[] | select(.body | startswith("## Codex Review")) | "- " + (.html_url // (.id | tostring))'
)

if [ -n "$codex_review_comments" ]; then
  echo "Codex Review comments require handling before merge:"
  echo "$codex_review_comments"
  exit 2
fi
...
- Codex review jobs retry on failure and are non-blocking; use the presence of
  ...
  that review feedback is available. The absence of such comments is also a
  valid signal: continue when CI and mergeability are green instead of waiting
  for an optional workflow that may not exist.
...
- Only request a new Codex review when the repository has that workflow and you
  need a rerun (e.g., after new commits). Do not request one without changes
  ...
  - Before requesting a new Codex review, re-run the land watcher and ensure
    ...
    replies).
  - After pushing new commits, repositories with a Codex review workflow may
    rerun it on PR synchronization (or you can re-run the workflow manually).
    Post a concise root-level summary comment so reviewers have the latest
    delta:
    ```
  ...
  - Only request a new review if there is at least one new commit since the
    previous request.
  - Query for the next Codex review comment before merging. If no
    `## Codex Review` comment appears and CI/mergeability are green, proceed;
    do not wait indefinitely for an optional workflow.
