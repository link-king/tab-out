...
- Canonical branch: `main` on `https://github.com/link-king/tab-out.git`.
- Keep product changes narrowly scoped to the Linear issue and preserve the extension's local-only privacy model.
- Never commit `extension/config.local.js`, `.DS_Store`, browser profiles, or user-specific Chrome data.
...
- For UI changes, also load `extension/` as an unpacked Chrome extension and exercise the affected new-tab and side-panel flows.
- Deliver through a feature branch and GitHub pull request using the repository Codex skills.
